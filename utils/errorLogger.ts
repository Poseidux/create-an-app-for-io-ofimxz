      ...
      fetch(url, {
        ...
        if (!fetchErrorLogged) {
          ...
          if (typeof window !== 'undefined' && window.console) {
            (window.console as any).__proto__.log.call(console, '[ErrorLogger] Fetch error (will not repeat):', e.message || e);
          }
        }
      });
    } catch (e) {
...
export const setupErrorLogging = () => {
  ...
  originalConsoleLog('[ErrorLogger] Setting up error logging...');
  originalConsoleLog('[ErrorLogger] Log server URL:', logServerUrl || 'NOT AVAILABLE');
  originalConsoleLog('[ErrorLogger] Platform:', Platform.OS);
